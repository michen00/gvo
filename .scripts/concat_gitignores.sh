...
Concatenate multiple .gitignore gvos into a single file by fetching
gvo URLs from stdin, a file, or built-in defaults.
...
{
  echo "$HEADER"
  echo "# This .gitignore is composed of the following gvos (retrieved $(date +%Y-%m-%d)):"
  for url in "${URLS[@]}"; do
    echo "# - $url"
  done
  echo "$HEADER"
